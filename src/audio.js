...
export class AudioSystem{
  constructor(){this.context=null;this.volume=.6;this.musicClock=0;this.musicStep=0;this.playing=false;this.biome=0;this.pickupChain=0;this.pickupClock=0}
  start(){
  ...
  effect(name,combo=0){
    ...
    if(name==='kill')this.tone(150+Math.min(combo,20)*13,.17,'sine',.09,1.8);
    if(name==='crit'){this.tone(720,.11,'triangle',.065,1.35);this.tone(330,.09,'sine',.04,.8)}
    if(name==='streak'){this.tone(440,.17,'triangle',.09,1.5);setTimeout(()=>this.tone(660,.23,'sine',.07,1.25),95)}
    if(name==='level'){this.tone(523,.14,'sine',.08,1);setTimeout(()=>this.tone(784,.2,'sine',.085,1),110)}
    ...
    if(name==='pickup'){
      this.pickupChain=this.pickupClock>0?Math.min(8,this.pickupChain+1):0;
      this.pickupClock=.7;
      this.tone(500+this.pickupChain*65,.1,'sine',.05,1.3);
    }
    if(name==='sweep'){this.tone(440,.13,'sine',.065,1.4);setTimeout(()=>this.tone(740,.22,'sine',.08,1.2),95)}
  ...
  update(dt){
    this.pickupClock=Math.max(0,this.pickupClock-dt);
